...
from src.rewards.grpo_math_reward_v4_strict import gsm8k_answer_reward_v4_strict
...
def build_grpo_trainer(model, tokenizer, train_dataset, training_args):
    ...
    if "reward_funcs" in sig:
        trainer_kwargs["reward_funcs"] = gsm8k_answer_reward_v4_strict
    elif "reward_func" in sig:
        trainer_kwargs["reward_func"] = gsm8k_answer_reward_v4_strict
    else:
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/grpo_v4_from_sft_v5_hard_strict_300.yaml")
    ...
    print("====== 加载 GRPO_v4 hard-prompt 数据 ======")
