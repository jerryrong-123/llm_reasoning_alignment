...
BASE_MODEL_PATH = "/root/autodl-tmp/models/Qwen/Qwen2___5-7B-Instruct"
ADAPTER_PATH = PROJECT_ROOT / "outputs" / "checkpoints" / "qwen25_7b_rag_sft_lora_full"
...
OUTPUT_FILES = {
    "top10_original_recheck": GENERATION_DIR / "rag_answers_qwen25_7b_lora_sft_full_top10_original_recheck.jsonl",
    "top10_soft_cap2_compressed": GENERATION_DIR / "rag_answers_qwen25_7b_lora_sft_full_top10_soft_cap2_compressed.jsonl",
    "top7_soft_cap2_compressed": GENERATION_DIR / "rag_answers_qwen25_7b_lora_sft_full_top7_soft_cap2_compressed.jsonl",
}

METRICS_PATH = EVAL_DIR / "rag_answer_generation_qwen25_7b_lora_sft_full_metrics.json"
REPORT_PATH = REPORT_DIR / "rag_answer_generation_qwen25_7b_lora_sft_full_report.md"
...
def load_model():
    ...
    print("base_model:", BASE_MODEL_PATH, flush=True)
    print("adapter:", ADAPTER_PATH, flush=True)
    ...
    print("====== 加载 LoRA adapter ======", flush=True)
    ...
    print("====== base model + LoRA adapter 加载完成 ======", flush=True)
...
def main():
    print("====== Qwen2.5-7B LoRA SFT Full RAG Evaluation ======", flush=True)
    ...
    all_metrics = {
        "method": "qwen25_7b_lora_sft_full_rag_evaluation",
        "model": "Qwen2.5-7B-Instruct + qwen25_7b_rag_sft_lora_full",
    ...
    print("====== Qwen2.5-7B LoRA SFT Full 生成完成 ======", flush=True)
    print("metrics:", METRICS_PATH, flush=True)
    print("report:", REPORT_PATH, flush=True)

    print("====== Qwen2.5-7B LoRA SFT Full 生成指标汇总 ======", flush=True)
    for name, metrics in all_metrics["variants"].items():
        print(
            f"{name}: "
