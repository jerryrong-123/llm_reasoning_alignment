...
from src.rewards.grpo_math_reward_v2 import gsm8k_answer_reward_v2
...
def build_grpo_trainer(model, tokenizer, train_dataset, training_args):
    ...
    if "reward_funcs" in sig:
       trainer_kwargs["reward_funcs"] = gsm8k_answer_reward_v2
    elif "reward_func" in sig:
       trainer_kwargs["reward_func"] = gsm8k_answer_reward_v2
    else:
        raise TypeError(
            "当前 TRL 的 GRPOTrainer 没有 reward_funcs / reward_func 参数，请检查 TRL 版本。"
        )
...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/grpo_format_reward_from_sft_v2_debug.yaml",
    )
    ...
    if not os.path.exists(start_adapter_path):
        raise FileNotFoundError(
            f"没有找到起始 LoRA adapter: {start_adapter_path}\n"
            f"请先确认 adapter checkpoint 是否存在。"
        )

    if not os.path.exists(train_file):
        raise FileNotFoundError(
            f"没有找到 GRPO 训练数据: {train_file}\n"
            f"请先运行: python scripts/33_prepare_grpo_format_reward_data.py"
        )

    os.makedirs(output_dir, exist_ok=True)

    print("====== 加载 GRPO/RLVR format-reward 数据 ======")
