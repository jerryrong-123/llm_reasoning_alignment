...
OUTPUT_PATH = PROJECT_ROOT / "data" / "processed" / "sft_debug.jsonl"
PREVIEW_PATH = PROJECT_ROOT / "data" / "samples" / "sft_debug_preview.jsonl"
...
def build_sft_text(question: str, assistant_response: str) -> str:
    """
    构造成 Qwen chat template 风格的 SFT 文本。
    后面 SFTTrainer 会直接读 text 字段。
    """
...
def prepare_gsm8k(max_samples: int = 20):
    print("\n====== 处理 GSM8K ======")
...
def prepare_math_algebra(max_samples: int = 20):
    print("\n====== 处理 MATH algebra ======")
...
def prepare_openr1_math(max_samples: int = 20):
    """
    OpenR1-Math-220k 的字段可能会比 GSM8K / MATH 更复杂。
    这里先做兼容式抽取：
    question/problem/prompt/instruction 作为题目；
    solution/answer/response/completion 作为推理答案。
    如果字段不匹配，就跳过，并打印字段让你后面精修。
    """
    print("\n====== 处理 OpenR1-Math-220k ======")
    ...
    for sample in ds:
        ...
        if not final_answer:
            # OpenR1 里有些样本不一定是 boxed 格式，这里先保守跳过
            skipped += 1
            continue
...
def main():
    ...
    os.makedirs(PROJECT_ROOT / "data" / "samples", exist_ok=True)

    all_records = []

    all_records.extend(prepare_gsm8k(max_samples=20))
    all_records.extend(prepare_math_algebra(max_samples=20))

    try:
        all_records.extend(prepare_openr1_math(max_samples=20))
    except Exception as e:
        print("OpenR1 处理失败，先跳过。错误信息:")
        print(repr(e))

    save_jsonl(all_records, OUTPUT_PATH)
    save_jsonl(all_records[:5], PREVIEW_PATH)
