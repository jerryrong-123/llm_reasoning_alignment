...
START_ADAPTER_PATH = PROJECT_ROOT / "outputs" / "checkpoints" / "sft_lora_small_v2"
TRAIN_FILE = PROJECT_ROOT / "data" / "processed" / "grpo_format_reward_debug.jsonl"

OUT_DIR = PROJECT_ROOT / "outputs" / "reports"
OUT_CSV = OUT_DIR / "sft_v2_format_reward_inspection.csv"
OUT_JSONL = OUT_DIR / "sft_v2_format_reward_inspection.jsonl"
OUT_MD = OUT_DIR / "sft_v2_format_reward_inspection.md"
...
def write_reports(rows):
    ...
    lines.append("# SFT small_v2 format reward inspection")
    lines.append("")
    lines.append("## 实验目的")
    lines.append("")
    lines.append("本报告检查从 `sft_lora_small_v2` 出发时，为什么 GRPO format-reward debug 只得到 reward=0.1 且 reward_std=0。")
    lines.append("")
    lines.append("本脚本不训练模型，只采样生成并计算 reward 组成。")
    lines.append("")
    ...
    lines.append(f"start_adapter_path: {START_ADAPTER_PATH.relative_to(PROJECT_ROOT)}")
    ...
    if len(reward_values) <= 1:
        lines.append("所有 inspected completions 的 reward 完全相同，这解释了为什么 GRPO debug 没有 advantage signal。")
    else:
        lines.append("inspected completions 出现了不同 reward，说明通过采样和更长生成可以产生 reward 方差。")
    lines.append("")
    lines.append("如果 format_hit 仍然很低，说明模型没有稳定遵守 `Final answer: <answer>` 格式。")
    lines.append("如果 correctness 仍然很低，说明主要问题不是格式，而是推理或计算错误。")
    lines.append("")
...
def main():
    if not START_ADAPTER_PATH.exists():
        raise FileNotFoundError(f"Missing start adapter: {START_ADAPTER_PATH}")
    ...
    print("====== 加载 SFT small_v2 LoRA adapter ======")
    model = PeftModel.from_pretrained(
        base_model,
        str(START_ADAPTER_PATH),
        is_trainable=False,
    )
    ...
    print("====== 开始 SFT-v2 reward inspection ======")
    ...
    print("====== SFT-v2 reward inspection done ======")
    print(f"csv:   {OUT_CSV}")
