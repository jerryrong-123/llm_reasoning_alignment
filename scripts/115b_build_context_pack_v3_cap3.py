...
OUT_TOP10_SOFT_cap3 = DATA_DIR / "rag_inputs_v3_top10_soft_cap3.jsonl"
OUT_TOP10_SOFT_cap3_COMPRESSED = DATA_DIR / "rag_inputs_v3_top10_soft_cap3_compressed.jsonl"
OUT_TOP7_SOFT_cap3_COMPRESSED = DATA_DIR / "rag_inputs_v3_top7_soft_cap3_compressed.jsonl"

METRICS_PATH = EVAL_DIR / "context_pack_v3_cap3_metrics.json"
REPORT_PATH = REPORT_DIR / "context_pack_v3_cap3_report.md"
...
def require_file(path: Path):
    if not path.exists():
        raise FileNotFoundError(f"缂哄皯鏂囦欢: {path}")
...
def extract_capitalized_phrases(text):
    """
    绮楃暐鎶藉彇 query 閲岀殑鑻辨枃瀹炰綋鐭銆?
    涓嶇敤 gold answer锛屽彧鐢?query 鏈韩锛屾墍浠ヤ笉娉勬紡绛旀銆?
    """
    text = str(text)
...
def sentence_score(query, title, sentence):
    ...
    # query 閲岀殑澶у啓瀹炰綋鐭濡傛灉鍑虹幇鍦ㄥ彞瀛愰噷锛屽姞鍒?
    for phrase in extract_capitalized_phrases(query):
        if phrase.lower() in sentence.lower():
            score += 1.5

    # title 鍑虹幇鍦ㄥ彞瀛愰噷锛屽姞涓€鐐瑰垎
...
def compress_context_text(query, title, text, max_sentences=3, max_chars=950):
    """
    鍙敤 query/title 閫夋嫨鍙ュ瓙锛屼笉鐢?ground_truth锛屼笉鐢?gold ids銆?
    v3 姣?v2 淇濆畧锛氭渶澶氫繚鐣?3 鍙ャ€?50 chars锛岄伩鍏嶆妸绛旀鍙ュ垹鎺夈€?
    """
...
def soft_parent_cap_contexts(contexts, max_per_parent=2):
    """
    soft parent dedup:
    姣忎釜 parent 鏈€澶氫繚鐣?max_per_parent 涓?child銆?
    鐩告瘮 v2 鐨?hard dedup锛屾瘡涓?parent 鍙繚鐣?1 涓紝杩欓噷鏇撮€傚悎 HotpotQA 澶氳烦璇佹嵁銆?
    """
    parent_counts = {}
    selected = []
...
def context_answerability_proxy(ground_truth, contexts):
    """
    鍙敤浜庤瘎浼帮紝涓嶇敤浜庡帇缂╁拰閫夋嫨銆?
    妫€鏌ュ帇缂╁悗鐨?contexts 閲屾槸鍚﹁繕鑳界湅鍒扮瓟妗堝瓧绗︿覆鎴栫瓟妗堝叧閿瘝銆?
    """
...
def main():
    ...
    variants = [
        ...
        {
            "name": "top10_soft_cap3",
            "top_k": 10,
            "use_soft_cap": True,
            "max_per_parent": 3,
            "use_compression": False,
            "output_path": OUT_TOP10_SOFT_cap3,
        },
        {
            "name": "top10_soft_cap3_compressed",
            "top_k": 10,
            "use_soft_cap": True,
            "max_per_parent": 3,
            "use_compression": True,
            "output_path": OUT_TOP10_SOFT_cap3_COMPRESSED,
        },
        {
            "name": "top7_soft_cap3_compressed",
            "top_k": 7,
            "use_soft_cap": True,
            "max_per_parent": 3,
            "use_compression": True,
            "output_path": OUT_TOP7_SOFT_cap3_COMPRESSED,
        },
    ]
    ...
    all_metrics = {
        ...
        "note": "v3 replaces hard parent dedup with soft parent cap=3 and adds compression-only variants plus answerability proxy.",
        "variants": {},
    }

    report_lines = []
    report_lines.append("# Context Pack v3 cap3 Report")
    report_lines.append("")
    report_lines.append("## 1. Purpose")
    report_lines.append("")
    report_lines.append("Context Pack v2 showed that hard parent deduplication reduced evidence recall. This v3 experiment uses soft parent cap=3 and compression-only variants to balance recall, precision, answerability, and context length.")
    report_lines.append("")
    ...
    report_lines.append("- `top10_soft_cap3` reduces repeated chunks from the same parent while allowing up to two chunks per parent.")
    report_lines.append("- `top10_soft_cap3_compressed` combines soft parent cap and compression.")
    report_lines.append("- `top7_soft_cap3_compressed` tests a shorter context budget for weaker generators.")
    report_lines.append("")
    ...
    print("====== Context Pack v3 cap3 瀹屾垚 ======")
    print("metrics:", METRICS_PATH)
    print("report:", REPORT_PATH)

    print("====== Context Pack v3 cap3 姹囨€?======")
